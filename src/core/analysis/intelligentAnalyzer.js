...
function generateRecommendations(analysis, projectInfo) {
  ...
  if (analysis.detectedFunctionalities.length === 0) {
    recommendations.push({
      type: "warning",
      message:
        "No specific functionalities detected. Consider adding more detailed requirements.",
      priority: "High",
    });
  }
  ...
  if (analysis.edgeCases.includes("boundary")) {
    recommendations.push({
      type: "suggestion",
      message:
        "Include boundary value testing for limits and ranges mentioned in requirements.",
      priority: "Medium",
    });
  }
  ...
  if (
    ...
    recommendations.push({
      type: "suggestion",
      message:
        "Consider performance and load testing for the identified functionalities.",
      priority: "Medium",
    });
  }
  ...
  if (analysis.detectedFunctionalities.some((f) => f.type === "api")) {
    recommendations.push({
      type: "suggestion",
      message:
        "Include API testing with focus on request/response validation and error codes.",
      priority: "High",
    });
  }
  ...
  if (analysis.detectedFunctionalities.some((f) => f.type === "validation")) {
    recommendations.push({
      type: "suggestion",
      message:
        "Include positive and negative validation test cases with various input formats.",
      priority: "High",
    });
  }
...
function estimateTestCases(analysis, sentenceCount) {
  let baseCount = Math.max(
    sentenceCount,
    analysis.detectedFunctionalities.length * 3,
  );
...
export function extractAcceptanceCriteria(text) {
  ...
  lines.forEach((line, index) => {
    ...
    for (
      let patternIndex = 0;
      patternIndex < acPatterns.length;
      patternIndex++
    ) {
...
function detectACType(text) {
  ...
  if (lower.startsWith("given") || lower.startsWith("dado"))
    return "precondition";
  if (lower.startsWith("when") || lower.startsWith("cuando")) return "action";
  if (lower.startsWith("then") || lower.startsWith("entonces"))
    return "expected";
  ...
  try {
    ...
    if (acceptanceCriteria.length > 0) {
      ...
      for (let acIndex = 0; acIndex < acceptanceCriteria.length; acIndex++) {
        ...
        console.log(
          `Processing AC ${acId} (index ${acIndex}): ${ac.text.substring(0, 50)}...`,
        );
        ...
        const generatedForThisAC = testCases.filter(
          (tc) => tc.acId === acId,
        ).length;
        console.log(
      ...
      if (
        analysis.detectedFunctionalities &&
        analysis.detectedFunctionalities.length > 0
      ) {
            ...
            if (func && func.scenarios && Array.isArray(func.scenarios)) {
              ...
              scenariosToUse.forEach((scenario, sIndex) => {
                ...
                if (
                  ...
                  !testCases.some(
                    (tc) =>
                      tc.title.toLowerCase() === testCase.title.toLowerCase(),
                  )
                ) {
                  testCases.push(testCase);
                }
      ...
      if (testCases.length === 0 && sentences.length > 0) {
        ...
        sentencesToUse.forEach((sentence, index) => {
          ...
          const testCase = createTestCaseFromSentence(
            sentence,
            functionality,
            format,
            index + 1,
    ...
    if (validTestsPerAC > 1 && acceptanceCriteria.length > 0) {
      ...
      const maxEdgeCases = Math.min(
        ...
        Math.max(
          0,
          acceptanceCriteria.length * validTestsPerAC - testCases.length,
        ),
      );
      if (
        analysis.edgeCases &&
        ...
        analysis.edgeCases.slice(0, maxEdgeCases).forEach((edgeCase, index) => {
          const testCase = createEdgeCaseTest(
            edgeCase,
            format,
            testCases.length + 1,
          );
          if (
            testCase &&
            !testCases.some(
              (tc) => tc.title.toLowerCase() === testCase.title.toLowerCase(),
            )
          ) {
            testCases.push(testCase);
          }
      ...
      if (analysis.edgeCases && Array.isArray(analysis.edgeCases)) {
        analysis.edgeCases
          .slice(0, validTestsPerAC)
          .forEach((edgeCase, index) => {
            const testCase = createEdgeCaseTest(
              edgeCase,
              format,
              testCases.length + 1,
...
async function createTestCaseFromAC(
  ...
  if (!ac || !ac.text) {
    ...
    return {
      ...
      type: "Functional",
      preconditions:
        "User has accessed the application; System is ready and configured",
      steps:
  ...
  if (typeof ac.text !== "string" || ac.text.trim().length === 0) {
    console.error(
      `createTestCaseFromAC: AC text is empty or not a string. AC:`,
      ac,
    );
  ...
  const priority =
    functionality?.priorities?.default || determinePriority(ac.text);
  // Always default to 'Functional' unless explicitly specified, and prefer first testType from array
  const testType =
    functionality?.testTypes?.[0] || functionality?.testType || "Functional";
  ...
  if (totalVariations > 1 && variationIndex > 0) {
    ...
    if (useAIForTitle) {
      try {
        ...
        console.warn(
          "AI title generation for variation failed, using fallback:",
          error,
        );
        title = generateTitleFromAC(acText);
      }
  ...
  const givenMatch = acText.match(
    /(?:Given|Dado)\s+(.+?)(?:\s+When|\s+Cuando|$)/i,
  );
  const whenMatch = acText.match(
    /(?:When|Cuando)\s+(.+?)(?:\s+Then|\s+Entonces|$)/i,
  );
  ...
  if (!expectedResult || expectedResult.trim().length === 0) {
    ...
    expectedResult =
      "The operation completes successfully and the expected behavior is verified.";
  }
  ...
  return {
    id,
    title,
    priority: adjustPriorityForVariation(
      priority,
      variationIndex,
      totalVariations,
    ),
 ...
 */
function adjustPriorityForVariation(
  basePriority,
  variationIndex,
  totalVariations,
) {
...
function createTestCaseFromScenario(scenario, functionality, format, id) {
  ...
  const givenMatch = scenario.match(
    /(?:Given|Dado)\s+(.+?)(?:\s+When|\s+Cuando|$)/i,
  );
  const whenMatch = scenario.match(
    /(?:When|Cuando)\s+(.+?)(?:\s+Then|\s+Entonces|$)/i,
  );
...
function createTestCaseFromSentence(sentence, functionality, format, id) {
  ...
  const givenMatch = sentence.match(
    /(?:Given|Dado)\s+(.+?)(?:\s+When|\s+Cuando|$)/i,
  );
  const whenMatch = sentence.match(
    /(?:When|Cuando)\s+(.+?)(?:\s+Then|\s+Entonces|$)/i,
  );
...
function createEdgeCaseTest(edgeCase, format, id) {
  ...
  function generateIntelligentSteps(edgeCaseType, format) {
    ...
    if (stepMap[edgeCaseType]) {
      return format === "gherkin"
        ? stepMap[edgeCaseType].gherkin
        : stepMap[edgeCaseType].stepByStep;
    }
  ...
  function generateIntelligentExpected(edgeCaseType) {
    const expectedMap = {
      ...
      security:
        "Access is properly restricted and appropriate security messages are displayed.",
      performance:
      ...
      timeout:
        "System handles timeout scenarios correctly and maintains session integrity.",
      concurrent:
        "System handles concurrent operations correctly without conflicts or data corruption.",
      network:
        "System handles network failures gracefully and maintains data consistency.",
    };
  ...
  const givenMatch = steps.match(
    /(?:Given|Dado)\s+(.+?)(?:\s+When|\s+Cuando|$)/i,
  );
  const whenMatch = steps.match(
    /(?:When|Cuando)\s+(.+?)(?:\s+Then|\s+Entonces|$)/i,
  );
  ...
  return {
    ...
    preconditions:
      "User has accessed the application; System is ready and configured for testing",
    steps,
...
function generateTitleFromAC(text) {
  ...
  title = title.replace(
    /(?:Given|When|Then|And|But|Dado|Cuando|Entonces|Y|Pero)\s+/gi,
    " ",
  );
  ...
  const actionPatterns = [
    // View/Display/Browse patterns with entity
    {
      ...
      format: (match) => {
        ...
        if (
          extractedEntity &&
          !obj.toLowerCase().includes(extractedEntity.toLowerCase())
        ) {
          obj = extractedEntity;
        }
        ...
        obj = obj
          .replace(
            /\b(list|page|section|view|details|information|data)\b/gi,
            "",
          )
          .trim();
        if (obj && obj.length > 2) {
    ...
    {
      pattern:
        /(?:create|add|new)\s+(?:a\s+)?(?:new\s+)?([a-z\s]+?)(?:$|\.|,|record|item|entry)/i,
      format: (match) => {
        let obj = match[1].trim().replace(/\s+/g, " ");
        if (
          extractedEntity &&
    ...
    {
      pattern:
        /(?:delete|remove)\s+(?:the\s+)?([a-z\s]+?)(?:$|\.|,|record|item|entry)/i,
      format: (match) => {
        let obj = match[1].trim().replace(/\s+/g, " ");
        if (
          extractedEntity &&
        ...
        return extractedEntity
          ? `Delete ${capitalizeTitleCase(extractedEntity)}`
          : "Delete Record";
      },
    },
    ...
    {
      pattern:
        /(?:verify|test|check|validate)\s+(?:that\s+)?(.+?)(?:$|\.|,|is|are|should|can)/i,
      format: (match) => {
  ...
  if (!matched) {
    ...
    title = title.replace(
      /^(the|a|an|i|i should|i can|i want|user can|user should)\s+/i,
      "",
    );
    ...
    if (sentences.length > 0) {
      ...
      const actionMatch = meaningfulPart.match(
        /(?:should|must|will|can|may)\s+(.+?)(?:$|\.|,)/i,
      );
      if (actionMatch) {
...
function generatePreconditions(text, functionality) {
  ...
  if (lowerText.includes("user") || lowerText.includes("usuario")) {
    if (
      ...
      preconditions.push(
        "Valid user account exists in the system with appropriate permissions",
      );
      ...
      preconditions.push(
        "User account exists in the system with required permissions",
      );
    }
  }
  ...
  if (
    ...
    if (
      ...
      preconditions.push(
        "At least one record exists in the system that can be modified",
      );
      preconditions.push("User has edit permissions for the record");
    } else if (lowerText.includes("delete") || lowerText.includes("remove")) {
      preconditions.push(
        "At least one record exists in the system that can be deleted",
      );
...
function generateGherkinFromAC(
  ...
  const patternGherkin = getPatternBasedSteps(
    text,
    functionality,
    "gherkin",
    variationIndex,
  ...
  const gherkinLines = text
    .split("\n")
    .filter((line) =>
      /^(Given|When|Then|And|But|Dado|Cuando|Entonces|Y|Pero)\s+/i.test(
        line.trim(),
      ),
    );
  ...
  if (acType === "action" || lowerText.startsWith("when")) {
    ...
    const givenStep =
      extractIntelligentGivenStep(text) || "The System Is In A Valid State";
    const thenStep = extractIntelligentThenStep(text);
...
function extractIntelligentWhenStep(text) {
  ...
  const actionPatterns = [
    ...
    {
      pattern:
        /(?:enter|type|fill|input)\s+(?:the\s+)?([a-z\s]+?)(?:\s+field|\s+input|$)/i,
  ...
  for (const pattern of expectedPatterns) {
    ...
    if (match && match[1] && match[1].trim().length > 5) {
      ...
      const cleaned = result
        .replace(/^(that|que|the|el|la|los|las)\s+/i, "")
        .trim();
...
function generateGherkinFromText(text) {
  ...
  if (thenMatch && thenMatch[1] && thenMatch[1].trim().length > 5) {
    ...
    // Clean up common prefixes
    thenText = thenText
      .replace(/^(that|que|the|el|la|los|las|I|should|must|will)\s+/i, "")
      .trim();
...
function generateStepsFromAC(
  ...
  if (totalVariations > 1) {
    if (variationIndex === 0) {
      ...
      if (functionality) {
        const patternSteps = getPatternBasedSteps(
          text,
          functionality,
          "stepByStep",
          variationIndex,
  ...
  const hasGherkin =
    /(?:Given|When|Then|And|But|Dado|Cuando|Entonces|Y|Pero)\s+/i.test(text);
...
function convertGherkinToStepByStep(gherkinText) {
  ...
  // Extract Given step with more context
  const givenMatch = gherkinText.match(
    /(?:Given|Dado)\s+(.+?)(?:\s+When|\s+Cuando|\n|$)/i,
  );
  if (givenMatch && givenMatch[1]) {
    ...
    if (lowerText.includes("page") || lowerText.includes("screen")) {
      ...
      if (pageMatch) {
        ...
        steps.push(
          `2. Verify the page loads correctly and all elements are visible`,
        );
      } else {
        steps.push(
          `1. Ensure ${givenText.charAt(0).toLowerCase() + givenText.slice(1)}`,
        );
      }
    } else if (
  ...
  const whenMatch = gherkinText.match(
    /(?:When|Cuando)\s+(.+?)(?:\s+Then|\s+Entonces|\n|$)/i,
  );
  if (whenMatch && whenMatch[1]) {
    ...
    if (whenLower.includes("click") || whenLower.includes("select")) {
      ...
      if (fieldMatch) {
        steps.push(
          `${steps.length + 1}. Locate the ${fieldMatch[1].trim()} field`,
        );
        steps.push(
          `${steps.length + 1}. Enter valid data in the ${fieldMatch[1].trim()} field`,
        );
      } else {
        steps.push(`${steps.length + 1}. ${capitalizeTitleCase(whenText)}`);
      }
  ...
  if (thenMatch && thenMatch[1]) {
    ...
    if (
      thenLower.includes("see") ||
      thenLower.includes("display") ||
      thenLower.includes("show")
    ) {
      const elementMatch = thenText.match(
        /(?:see|display|show)\s+(?:the\s+)?([a-z\s]+?)(?:\.|$)/i,
      );
      if (elementMatch) {
        steps.push(
        ...
        steps.push(
          `${steps.length + 1}. Verify all expected information is present and accurate`,
        );
      } else {
        steps.push(
        ...
        steps.push(
          `${steps.length + 1}. Confirm the expected outcome is achieved`,
        );
      }
    } else {
    ...
    steps.push(
      `${steps.length + 1}. Confirm the operation completed successfully`,
    );
  }
  ...
  switch (variationIndex) {
    case 1: // Positive path - comprehensive detailed steps
      ...
      if (
        ...
        positiveSteps.push(
          "1. Launch the application and authenticate if required",
        );
        positiveSteps.push(
          "2. Navigate to the main list or view page using the navigation menu",
        );
        positiveSteps.push(
        ...
        positiveSteps.push(
          "7. Wait for the detailed view page to load completely",
        );
        positiveSteps.push(
          "8. Verify all record details are displayed in the detail view panel",
        );
        positiveSteps.push(
        ...
        positiveSteps.push(
          "11. Verify all mandatory fields are populated and visible",
        );
        positiveSteps.push(
          "12. Verify optional fields display correctly when they contain data",
        );
        positiveSteps.push(
          "13. Test all interactive navigation elements (back, edit, delete buttons)",
        );
        positiveSteps.push(
          "14. Verify data sorting functionality works correctly if available",
        );
        positiveSteps.push(
          "15. Verify filtering options function as expected if applicable",
        );
        positiveSteps.push(
          "16. Test the responsive design by resizing the browser window",
        );
        positiveSteps.push(
          "17. Verify no data corruption or truncation is visible",
        );
        positiveSteps.push(
        ...
        positiveSteps.push(
          "3. Verify all input fields accept the entered values",
        );
        ...
        positiveSteps.push(
          "8. Verify system generates a unique identifier (if applicable)",
        );
      } else if (
        lowerText.includes("update") ||
        lowerText.includes("edit") ||
        ...
        positiveSteps.push(
          "4. Modify the desired fields with new valid values",
        );
        positiveSteps.push("5. Click on the 'Save' or 'Update' button");
        positiveSteps.push(
          "6. Verify the updated information is reflected immediately",
        );
        positiveSteps.push(
          "7. Verify previous values are replaced with new values",
        );
        ...
        positiveSteps.push(
          "5. Verify the record is removed from the list/view",
        );
        positiveSteps.push(
          "6. Verify deletion confirmation message is displayed",
        );
        positiveSteps.push(
          "7. Verify the record cannot be accessed after deletion",
        );
      } else {
        ...
        positiveSteps.push(
          `${baseStepsArray.length + 2}. Confirm all expected outcomes are met`,
        );
        positiveSteps.push(
    ...
    case 2: // Negative path - error scenarios with completely different steps
      ...
      if (
        ...
        negativeSteps.push(
          "3. Attempt to access a non-existent record ID or invalid URL",
        );
        negativeSteps.push(
          '4. Verify the system displays an appropriate error message (e.g., "Record not found")',
        );
        negativeSteps.push(
          "5. Verify the error message is clear and user-friendly",
        );
        negativeSteps.push(
        ...
        negativeSteps.push(
          "8. Attempt to access a record without proper permissions",
        );
        negativeSteps.push(
          "9. Verify the system displays an appropriate access denied message",
        );
        negativeSteps.push(
        ...
        negativeSteps.push(
          "2. Leave required fields empty or fill with invalid data",
        );
        negativeSteps.push(
          "3. Enter invalid format data (e.g., text in numeric fields, invalid email format)",
        );
        negativeSteps.push(
          "4. Attempt to submit the form without completing required fields",
        );
        negativeSteps.push(
        ...
        negativeSteps.push(
          "7. Verify the form is not submitted and remains on the same page",
        );
        ...
        negativeSteps.push(
          "10. Verify appropriate business rule violation error is displayed",
        );
        negativeSteps.push(
        ...
        negativeSteps.push(
          "2. Select a record that is locked or has dependencies",
        );
        negativeSteps.push("3. Attempt to edit the record");
        negativeSteps.push(
          "4. Verify the system displays an appropriate error message",
        );
        negativeSteps.push(
          "5. Attempt to update with invalid or incomplete data",
        );
        negativeSteps.push(
          "6. Leave required fields empty or enter invalid format data",
        );
        ...
        negativeSteps.push("1. Navigate to the list or view page");
        negativeSteps.push(
          "2. Select a record that has dependencies or is protected",
        );
        ...
        negativeSteps.push(
          "5. Verify the error message explains why the deletion is not allowed",
        );
        negativeSteps.push(
          "6. Verify the record is not deleted and remains in the system",
        );
        negativeSteps.push("7. Attempt to delete a non-existent record");
        negativeSteps.push(
          "8. Verify the system handles the request gracefully",
        );
        ...
        negativeSteps.push(
          "3. Verify the system displays an appropriate error message",
        );
        negativeSteps.push(
    ...
    case 3: // Edge case - boundary conditions with completely different steps
      ...
      if (
        ...
        edgeSteps.push(
          "3. Test viewing the first record in the list (boundary: first item)",
        );
        edgeSteps.push("4. Verify the first record displays correctly");
        edgeSteps.push(
          "5. Test viewing the last record in the list (boundary: last item)",
        );
        edgeSteps.push("6. Verify the last record displays correctly");
        edgeSteps.push(
          "7. Test viewing when the list is empty (boundary: no items)",
        );
        edgeSteps.push(
          "8. Verify appropriate empty state message is displayed",
        );
        edgeSteps.push(
        ...
        edgeSteps.push(
          "12. Verify text truncation or wrapping is handled correctly",
        );
        edgeSteps.push(
        ...
        edgeSteps.push(
          "2. Test with minimum allowed values (boundary: minimum)",
        );
        edgeSteps.push(
          "3. Enter the minimum valid value in each numeric field",
        );
        ...
        edgeSteps.push(
          "6. Test with maximum allowed values (boundary: maximum)",
        );
        edgeSteps.push(
          "7. Enter the maximum valid value in each numeric field",
        );
        ...
        edgeSteps.push(
          "10. Test with special characters and edge case formats",
        );
        edgeSteps.push(
          "11. Verify the system handles boundary values correctly",
        );
        edgeSteps.push(
          "12. Verify data integrity is maintained at boundary conditions",
        );
      } else if (
        lowerText.includes("update") ||
        lowerText.includes("edit") ||
        ...
        edgeSteps.push(
          "9. Test updating with special characters and edge case formats",
        );
        edgeSteps.push(
          "10. Verify the system handles boundary values correctly",
        );
        edgeSteps.push(
          "11. Verify data integrity is maintained at boundary conditions",
        );
      } else if (lowerText.includes("delete") || lowerText.includes("remove")) {
        edgeSteps.push("1. Navigate to the list or view page");
        edgeSteps.push(
          "2. Test deleting the first record in the list (boundary: first item)",
        );
        edgeSteps.push("3. Verify the deletion is successful");
        edgeSteps.push(
          "4. Test deleting the last record in the list (boundary: last item)",
        );
        edgeSteps.push("5. Verify the deletion is successful");
        edgeSteps.push(
          "6. Test deleting when only one record exists (boundary: single item)",
        );
        edgeSteps.push(
          "7. Verify appropriate empty state is displayed after deletion",
        );
        ...
        edgeSteps.push(
          "10. Verify system maintains stability at boundary conditions",
        );
      } else {
        ...
        edgeSteps.push(
          "5. Verify the system handles boundary conditions correctly",
        );
        edgeSteps.push(
          "6. Verify system maintains stability and data integrity",
        );
      }
    ...
    case 4: // Alternative flow - completely different approach
      ...
      if (
        ...
        altSteps.push(
          "3. Alternatively, use the search functionality to find and view the record",
        );
        altSteps.push(
          "4. Alternatively, access the record through a direct URL or bookmark",
        );
        altSteps.push(
          "5. Verify the record details are displayed correctly through the alternative method",
        );
        altSteps.push(
          "6. Verify all expected information is present regardless of access method",
        );
        altSteps.push(
          "7. Test accessing through mobile responsive view if applicable",
        );
        altSteps.push(
        ...
        altSteps.push(
          "2. Use alternative methods such as import functionality or bulk create",
        );
        altSteps.push(
          "3. Alternatively, use API or data import tools if available",
        );
        altSteps.push(
          "4. Alternatively, use copy/duplicate functionality to create similar records",
        );
        altSteps.push(
          "5. Verify the records are created successfully using the alternative method",
        );
        altSteps.push(
          "6. Verify all data is saved correctly regardless of creation method",
        );
        altSteps.push(
        ...
        altSteps.push(
          "3. Alternatively, use bulk update or mass edit features",
        );
        altSteps.push(
          "4. Alternatively, use keyboard shortcuts for quick edits",
        );
        altSteps.push(
          "5. Verify the updates are saved successfully using the alternative method",
        );
        ...
        altSteps.push("1. Navigate to the list or view page");
        altSteps.push(
          "2. Use bulk delete or multi-select functionality if available",
        );
        altSteps.push(
          "3. Alternatively, use context menu or right-click options to delete",
        );
        altSteps.push("4. Alternatively, use keyboard shortcuts for deletion");
        altSteps.push(
          "5. Verify the records are deleted successfully using the alternative method",
        );
        ...
        altSteps.push("1. Navigate to the application");
        altSteps.push(
          "2. Use alternative methods or approaches to perform the required action",
        );
        altSteps.push(
          "3. Verify the alternative approach is available and functional",
        );
        altSteps.push(
          "4. Verify the action completes successfully using the alternative method",
        );
        altSteps.push(
...
function generateDetailedStepsFromText(text, functionality = null) {
  ...
  if (pageMatch) {
    ...
    steps.push(
      `${stepNumber++}. Verify the page loads correctly and all UI elements are visible`,
    );
  } else if (
    lowerText.includes("login") ||
  ...
  const actionPatterns = [
    ...
    },
    {
      ...
      generate: (match) => {
        ...
        steps.push(
          `${stepNumber++}. Fill in all required fields with valid data`,
        );
    ...
    },
    {
      ...
      generate: (match) => {
        const item = match[1].trim();
        steps.push(
          `${stepNumber++}. Navigate to the ${item} list or detail page`,
        );
    ...
    },
    {
      ...
      generate: (match) => {
        ...
        steps.push(
          `${stepNumber++}. Confirm the deletion in the confirmation dialog`,
        );
      },
    },
    {
      ...
      generate: (match) => {
        ...
        steps.push(
          `${stepNumber++}. Click on the 'Search' button or press Enter`,
        );
      },
    },
    {
  ...
  if (!actionFound) {
    ...
    if (sentences.length > 0) {
      sentences.forEach((sentence) => {
        ...
        if (
          cleanSentence &&
          !cleanSentence.match(/^(I|user|usuario)\s+(?:am|is|are)/i)
        ) {
          steps.push(
  ...
  if (
    ...
    const expectedMatch = text.match(
      /(?:should|must|will|verify|check|confirm)\s+(.+?)(?:\.|$)/i,
    );
    if (expectedMatch) {
      steps.push(
        `${stepNumber++}. Verify that ${expectedMatch[1].trim().toLowerCase()}`,
      );
    } else {
...
function generateExpectedResult(
  ...
  if (!text || typeof text !== "string") {
    ...
    if (
      contextResult &&
      contextResult !==
        "The Expected Outcome Should Be Achieved And Verified Successfully"
    ) {
  ...
  if (functionality && functionality.type) {
    const patternExpected = getPatternBasedExpectedResult(
      text,
      functionality,
      variationIndex,
    );
  ...
  if (thenMatch && thenMatch[1]) {
    ...
    if (thenText.length > 5) {
      ...
      thenText = thenText
        .replace(/^(I|should|must|will|debe|será)\s+/i, "")
        .trim();
  ...
  if (
    source === "expected" ||
    lowerText.startsWith("then") ||
    lowerText.includes("then")
  ) {
    const thenPattern = text.match(
      /(?:Then|Entonces)[:\s]+(.+?)(?:\.|$|When|Given)/i,
    );
    if (thenPattern && thenPattern[1] && thenPattern[1].trim().length > 5) {
      return (
        thenPattern[1].trim().charAt(0).toUpperCase() +
        thenPattern[1].trim().slice(1)
      );
    }
  }
  ...
  if (shouldMatch && shouldMatch[1]) {
    ...
    resultText = resultText
      .replace(/\s+(showing|with|containing)\s*$/, "")
      .trim();
  ...
  const simpleShouldMatch = text.match(
    /(?:should|debe|must|will)\s+(.+?)(?:\.|$|and|or|,)/i,
  );
  if (simpleShouldMatch && simpleShouldMatch[1]) {
    let resultText = simpleShouldMatch[1].trim();
    // Skip if it's too generic
    if (
      !resultText.match(/^(be|see|view|display|show|get|receive)$/i) &&
      resultText.length > 5
    ) {
      resultText = resultText.replace(/,\s*$/, "").trim();
      resultText = resultText.charAt(0).toUpperCase() + resultText.slice(1);
  ...
  const expectedMatch = text.match(
    /(?:expected|esperado|result)[:\s]+(.+?)(?:\.|$)/i,
  );
  if (expectedMatch && expectedMatch[1] && expectedMatch[1].trim().length > 5) {
    return (
      expectedMatch[1].trim().charAt(0).toUpperCase() +
      expectedMatch[1].trim().slice(1)
    );
  }
  ...
  if (clickEntityMatch && clickEntityMatch[1]) {
    ...
    const resultMatch = text.match(
      /(?:should|will|must|then)\s+(.+?)(?:\.|$|and|or)/i,
    );
    if (resultMatch && resultMatch[1]) {
      let resultText = resultMatch[1].trim();
      resultText = resultText
        .replace(/^(I|be|see|view|display)\s+/i, "")
        .trim();
      if (resultText.length > 5) {
    ...
    switch (funcType) {
      ...
      case "crud":
        if (
          ...
          funcText.includes("add") ||
          funcText.includes("new")
        ) {
  ...
  if (steps) {
    ...
    if (thenStepMatch && thenStepMatch[1]) {
      ...
      if (
        ...
        const cleaned = thenText
          .replace(/^(I\s+(?:should|will|must|can)|The|That)\s+/i, "")
          .trim();
    ...
    if (steps.includes("\n")) {
      ...
      if (stepLines.length > 0) {
        ...
        if (
          lastStep.match(
            /(?:verify|check|confirm|validate|ensure|expect|should|see|display|show)/i,
          )
        ) {
  ...
  if (title) {
    ...
    const actionPatterns = [
      ...
      },
      {
        pattern: /(?:login|sign in|authenticate)/i,
        result:
          "User is successfully authenticated and redirected to the appropriate page",
      },
      {
      ...
      {
        pattern: /(?:download|export)/i,
        result: "File is successfully downloaded/exported",
      },
      {
        pattern: /(?:validate|verify|check)/i,
        result:
          "Validation is performed correctly and appropriate feedback is provided",
      },
    ];
  ...
  const actionVerbs = {
    ...
    validate:
      "Input is validated correctly and appropriate feedback is provided",
 ...
 */
function getPatternBasedExpectedResult(
  text,
  functionality,
  variationIndex = 0,
) {
  ...
  if (variationIndex === 0 || variationIndex === 1) {
    ...
    if (
      genericPatterns &&
      genericPatterns.basicFlow &&
      genericPatterns.basicFlow.expectedResult
    ) {
 ...
 */
export function generateIntelligentItemName(
  text,
  planType = "comprehensive",
  index = 0,
) {
  ...
  let name = cleanText
    ...
    .replace(
      /^(as a|como|user|usuario|i want|quiero|so that|para que)[:-\s]+/i,
      "",
    )
    .replace(
      /^(ac|acceptance criteria|criterio de aceptación|requirement|requisito)[:-\s]+/i,
      "",
    )
    .replace(/^#+\s+/, "")
    .replace(/^-\s+/, "")
  ...
  if (analysis && analysis.detectedFunctionalities.length > 0) {
    ...
    if (func.type === "authentication") {
      if (lowerText.includes("login") || lowerText.includes("iniciar sesión")) {
        ...
        lowerText.includes("cerrar sesión")
      ) {
        name = "User Logout";
      } else if (
        lowerText.includes("password") ||
        lowerText.includes("contraseña")
      ) {
