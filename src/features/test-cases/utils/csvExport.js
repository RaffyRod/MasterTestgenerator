...
const formatMappings = {
  ...
  testmo: {
    ...
    formatRow: (testCase, isFirstRow = true, stepIndex = 0, totalSteps = 0) => {
      ...
      const preconditions = formatHTMLText(
      ...
      if (isFirstRow) {
        ...
        return {
          ...
          Estimate: "",
          Expected: formatHTMLText(
            testCase.expectedResult || "Operation completes successfully",
          ),
          Folder: folderName,
          Forecast: "",
          Issues: "",
  ...
  if (numberedMatches && numberedMatches.length > 0) {
    numberedMatches.forEach((match) => {
      ...
      if (expectedMatch) {
        ...
        const step = parts[0] || "";
        const expected =
          parts.slice(1).join(" ").trim() || "Step completes successfully";
        if (step) {
          steps.push({ step, expected });
        }
      }
    ...
    if (gherkinSteps.length > 0) {
      ...
      groupedLines.forEach((line, index) => {
        if (line) {
          ...
          const expectedPattern =
            /(?:expected|result|should|debe)[:\s]+(.+?)(?:\.|$)/i;
...
function generateTestMOTags(testCase) {
  ...
  if (testCase.type) {
    ...
    if (
      typeLower.includes("functional") ||
      typeLower.includes("functionality")
    ) {
