...
describe("Bug Title Generation", () => {
  ...
  describe("generateIntelligentTitle for bug reports", () => {
    it("should return array of 2 titles for bug reports", async () => {
      const description =
        "The app is failing after reloading the main page when the user is logged off";
      ...
      const result = await generateIntelligentTitle(
        description,
        "en",
        "bugReport",
      );
    ...
    it("should limit titles to 30 characters", async () => {
      const description =
        "The application is not working properly when the user tries to access the dashboard page after logging in";
      ...
      global.fetch = vi
        ...
        .mockResolvedValueOnce({
          ...
          json: async () => ({
            generated_text:
              "This is a very long title that should be truncated to 30 characters maximum",
          }),
        })
        .mockResolvedValueOnce({
          ok: true,
            ...
            choices: [
              {
                message: {
                  content:
                    "Another very long title that exceeds the maximum allowed length of 30 characters",
                },
              },
            ],
          }),
    ...
    it("should use fallback when both AIs fail", async () => {
      ...
      // Suppress console warnings for expected API failures
  ...
  describe("Title selection functionality", () => {
    it("should return single title for non-bug-report types", async () => {
      const text =
        "Given I am logged in, When I click the button, Then I should see the page";
